...
> 🚀 **Mejoras post-v1 (2026-07-01).** Dos mejoras aditivas cerradas sobre el v1, ya en **develop + main + origin** (no alteran el roadmap ni el motor de conciliación):
> - **(a) Dashboard ejecutivo v2 — analítica temporal.** El dashboard de Fase 6 (foto de un periodo) se enriqueció con analítica en el tiempo: serie mensual, tendencias (ingresos/egresos/utilidad), márgenes en el tiempo, composición de egresos (categoría/naturaleza fijos vs variables), ingreso por empresa/MRR (Tu Checador), top proveedores y rollup de nómina. Nuevo `App\Services\Finance\FinanceAnalyticsService` (POPO team-explícito, sin migración, reusa `ProfitLossService`+`PeriodResolver`) + param `months` (6/12) en `/executive`; UI con **ApexCharts** (`vue3-apexcharts`) y tablas de serie/desgloses en el PDF (dompdf no hace charts). SDD: `docs/sdd/07-executive-dashboard.md` (§v2).
> - **(b) Catálogo cliente→empresa + auto-asignación de ingresos.** Nuevo modelo `ClienteEmpresa` (`cliente_empresas`) + `ClienteEmpresaService` que mantiene un mapeo auto-aprendido **RFC → empresa**: aprende al asignar empresa a un grupo (`updateGroupEmpresa`) y **auto-asigna** al conciliar (`store`/`batch`) cuando el mapeo es unívoco. Página `/clients` (catálogo editable + aplicar al histórico + detección de clientes recurrentes que "dejaron de facturar"). Único toque al motor: `MatcherService::reconcile()` ahora **devuelve el `group_id`** (algoritmo de matching intacto). Solo ingresos. SDD: `docs/sdd/08-catalogo-cliente-empresa.md`.

> 🚀 **Mejoras post-v1 (2026-07-06).** Tres cierres aditivos más, en **develop + main + origin**:
> - **(c) Exclusión por cliente del catálogo ("Respetar etiquetas").** Columna `cliente_empresas.excluido` + toggle en `/clients` para clientes genéricos multi-empresa (ej. público en general `XAXX010101000`): no aprende, no sugiere (bloqueante en la regla estricta), no se aplica al histórico; el mapeo queda inerte/reversible y la asignación manual por grupo sigue normal. `ClienteEmpresa::scopeAplicable()` centraliza el predicado. SDD: `docs/sdd/08-catalogo-cliente-empresa.md` (§exclusión); reglas en `business-rules.md` §14.7.
> - **(d) Rol `admin` del team (acceso CEO).** `team_user.role='admin'` es **owner-equivalente para los módulos financieros** (dashboard ejecutivo + export, empleados, tolerancia, mutaciones de empresas/categorías) vía `User::managesTeam`/`ChecksTeamOwnership::managesCurrentTeam`; la gestión del team (renombrar, invitar/quitar) sigue solo-owner. Comando `php artisan team:member-role {email} admin` (**activo en prod**: ventas@apscreativas.com). Sin migración. Docs: `security.md` §11, `operations.md`, `architecture.md`.
> - **(e) Fix de layout del header.** El slot `#header` de `AuthenticatedLayout` no tenía `flex-1` → los botones de acción quedaban pegados al título en los 6 módulos; corregido en el layout (una sola vez para todas las páginas).
...
### Fase 3B — Módulo de Empleados + nómina recurrente  ✅ CERRADA (2026-06-30)
> Implementada en `feature/finanzas-fase3b`. SDD: `docs/sdd/04-empleados-nomina.md`. Tabla `empleados` (TeamOwned) + CRUD `/employees` solo-owner (salarios sensibles); `egresos.empleado_id` + `concepto_nomina` + índice único `egresos_empleado_periodo_unique` + `user_id` nullable/nullOnDelete; `PayrollCalculator` (quincenas con día hábil) y comando `nomina:generar` quincenal idempotente (ventana móvil 40d, `--month`, `--dry-run`) + schedule diario 01:30. 20 tests nuevos verdes (PayrollCalculator 4, GenerarNomina 12, Empleado 4), 0 regresiones.

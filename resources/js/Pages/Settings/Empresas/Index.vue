...
const destroy = () => {
    ...
    router.delete(route("settings.empresas.destroy", toDelete.value.id), {
        preserveScroll: true,
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <Link v-if="isOwner" :href="route('settings.empresas.create')">
                            ...
                            <tr v-for="empresa in empresas" :key="empresa.id">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    <template v-if="isOwner">
                                        <Link :href="route('settings.empresas.edit', empresa.id)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400">{{ $t('Editar') }}</Link>
